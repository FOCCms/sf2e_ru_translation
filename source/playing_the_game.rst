...
.. sidebar:: Игровые соглашения (Game Conventions)
	...
	.. _sidebar--Game-Conventions--Duplicate-Effects:
...
Штраф множественной атаки (`Multiple Attack Penalty <https://2e.aonprd.com/Rules.aspx?ID=322>`_)
""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""

Во время своего хода, чем больше атак вы совершаете после первой, тем менее точным вы становитесь.
Когда вы используете :t_attack:`атакующее` действие второй раз во время своего хода, вы получаете штраф -5 к своей проверке.
Когда вы атакуете третий и более раз, то получаете к этим проверкам штраф -10.
Каждая проверка, имеющая признак :t_attack:`атака`, считается к вашему штрафу множественной атаки, включая :ref:`Удары (Strikes) <action--Strike>`, броски атак заклинаний, определенные действия навыков как :ref:`skill--Athletics--Shove` и многие другие.

.. versionchanged:: /errata-CR-r3
	Здесь была неконсистентность с остальными правилам, так как было написано, что ШМА дается "броскам атак", тем самым игнорируя проверки навыков с признаком :t_attack:`атака`, хотя последующий текст упоминает их.
	Перефразировано в "проверки".
...
Второй Удар будет иметь штраф -5 при использовании длинного меча или -4 с коротким мечом.
...
Как и обычный урон, он может быть удвоен или поделен пополам в зависимости от результатов броска атаки или спасброска.
В отличие от нормального урона, когда вы подвергаетесь продолжительному урону, то не получаете урон сразу.
Вместо этого, вы получаете определенный урон в конце своих ходов, после чего совершаете чистую проверку КС 15, чтобы узнать восстановились ли вы от продолжительного урона.
Для полного описания состояния продолжительного урона смотрите состояние :ref:`condition--Persistent-Damage` и его сноску.

.. versionchanged:: /errata-CR-r3
	Добавлено предложение об удвоении и делении пополам продолжительного урона в зависимости от результата действия эффекта.
...
.. sidebar:: Типы урона (Damage Types)
	...
	**Дробящий урон** наносится оружием или опасностями которые наносят травмы тупыми предметами, как удар дубиной или обвал камней.
...
.. _ch9--Damage--Immunity--Critical-Hit:
...
.. admonition:: Официальное пояснение (4 издание)
	...
	**Вопрос**: Как работают дополнительные критические эффекты на существ с иммунитетом к критическим попаданиям?

	**Ответ**: Работают так как там и написано - Атака наносит нормальный урон вместо удвоенного.
	Прочие эффекты специфичные для критического попадания все еще происходят, как например, эффект критической специализации и дополнительные кости урона от таких признаков как :w_deadly:`смертельное` и :w_fatal:`фатальное`.
	Вы все так же имеете вариант применять умения провоцируемые при критических попаданиях, как у :ref:`Стрижающей (Vorpal) <item--Vorpal>` руны (однако, многим существам иммунным к критам для жизни и голова не нужна; вот ведь везучие засранцы).
	В определенных случаях ваш Мастер все еще может отказать в эффектах при каких-либо чрезвычайно странных обстоятельствах.
...
.. _ch9--Damage--Resistance--all-damage:
...
Обычно для поддержания линии эффекта достаточно зазора в 1 квадратный фут (прим.пер: ~929 см :sup:`2`), однако последнее слово за Мастером.
...
Если вы не уверены насколько сплошной является преграда, то обычно для поддержания линии обзора достаточно зазора в 1 квадратный фут (~ 929 см :sup:`2`), однако последнее слово за Мастером.
...
.. sidebar:: Пример недуга (Affliction Example)
...
Если стадия снижается ниже 1, то недуг заканчивается и вам больше не надо совершать дальнейшие спасброски, если только вы снова не подвергнитесь недугу.
...
.. _ch9--Afflictions--Conditions-from-Afflictions:

Состояния от недугов (Conditions from Afflictions)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Недуги могут дать вам состояния с большей или меньшей продолжительностью, чем сам недуг.
...
.. admonition:: Официальное пояснение (4 издание)
	...
	**Вопрос**: Если недуг делает меня :c_enfeebled:`ослабленным 1` не указывая продолжительность и данный недуг завершается, то я остаюсь :c_enfeebled:`ослаблен` постоянно?

	**Ответ**: В данном разделе описывается, что состояние может длится дольше продолжительности вызвавшего его недуга, приводя в пример :c_drained:`истощение`.
	Существует 3 категории эффектов от недугов:

	#. Мгновенные эффекты, такие как урон происходят как только вы достигаете стадии.
	#. Состояния имеющие способ завершения по умолчанию, длятся нормальную продолжительность. К ним относятся такие состояния как :c_drained:`истощен`, :c_frightened:`напуган`, :c_sickened:`тошнота` и продолжительный урон.
	#. Состояния которым всегда нужно указывать продолжительность, так как нет нормального способа оправиться от них - как у :c_clumsy:`неуклюжести` или :c_paralyzed:`паралича` - длятся столько же, сколько и стадия недуга от которого они появились. Это также применяется к продолжающимся эффектам, но специфичных для недуга, а не определяемых состояниями, как штраф к определенным броскам.


.. _ch9--Afflictions--Multiple-Exposures:
...
После того как вы умираете, вы не можете действовать или восстанавливать действия, становиться целью заклинаний, которые нацелены на существ (если только они не выбирают целью конкретно мертвых существ), и по всем остальным признакам вы считаетесь объектом.
...
.. versionchanged:: /errata-CR-r4
	Добавлено про "восстановление действий".
...
Существо с быстрым исцелением или регенерацией восстанавливает указанное количество Очков Здоровья в начале своего хода.
...
Состояния могут изменять количество действий, которые вы можете использовать в ход или можете ли вы их вовсе использовать.
...
Когда вы теряете количество действий, будь то из-за состояний или иным путем, и если между ними есть какая-либо разница, то вы выбираете какое из них потерять.
...
Самые ограничивающие виды сокращения действий, это такие в которых эффект гласит, что вы совсем не можете действовать: это значит, что вы не можете использовать никакие действия, или даже :ref:`говорить <sidebar--Speaking>`.
Когда вы не можете действовать, вы все еще восстанавливаете действия в начале своего хода, если только этому не мешает состояние (такое как :c_stunned:`ошеломлен`).

.. versionchanged:: /errata-CR-r4
	В последнем предложении теперь говорится, что вы восстанавливаете действия, если только этому не мешает состояние.
...
Виды передвижения (`Movement Types <https://2e.aonprd.com/Rules.aspx?ID=396>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
Далее перечислены различные виды передвижения.
Так как действие :ref:`action--Stride` может быть использовано только с вашей нормальной Скоростью, передвижение с использованием одного из этих видов передвижения требует использования специального действия, и во время использования этих видов передвижения вы не можете :ref:`Шагнуть (Step) <action--Step>`.
Так как Скорость сама по себе ссылается на вашу наземную Скорость, то правила, касающиеся этих специальных видов передвижения, уточняют применяемые к ним виды передвижения.
Хотя Скорости не являются проверками, но они могут иметь бонусы предмета, обстоятельства и состояния, а так же штрафы.
Они не могут снизить вашу Скорость ниже 5 футов, если только не указано иное.

Переход с одного вида передвижения на другой требует закончить ваше действие имеющее первый вид передвижения и использовать действие имеющее второй вид передвижения.
Например, если вы :ref:`Вскарабкались (Climbed) <skill--Athletics--Climb>` на 10 футов до вершины уступа, то потом можете :ref:`Переместиться (Stride) <action--Stride>` на 10 футов вперед.
...
В общем, движение против ветра использует те же правила, что и движение по :ref:`сложной местности <ch9--Difficult-Terrain>` (или особо сложной местности, если вы еще и летите вверх), а движение по ветру позволяет вам двигаться на 10 футов за каждые потраченные 5 футов передвижения (не объединяется с движением строго вниз).
Для подробной информации о расходовании передвижения смотрите раздел :ref:`ch9--Enc--Movement-in-Encounters`.
...
В дополнение, если вы падаете в воду, снег или другое относительно мягкое вещество, то можете считать падение, как если бы оно было на 20 футов короче, или на 30 футов если вы ныряли умышленно.
...
.. sidebar:: Обнаружение другими чувствами (Detecting With Other Senses)
	...
	Большинство умений, который гласят "существо которое вы можете видеть" или тому подобное, нормально функционируют если персонаж может точно ощущать субъект каким-либо чувством.
	Если монстр использует чувство отличное от зрения, Мастер может адаптировать способы, которые не позволяют его противникам быть обнаруженными, к эквивалентам, которые работают с чувствами монстра.
	Например, существо с эхолокацией может использовать слух как основное чувство.
	Это значит, что его добыча будет :c_concealed:`скрыта` в шумном помещении, :c_hidden:`спрятана` при сильном грохоте, или даже :c_invisible:`невидима` под действием заклинания :ref:`spell--s--Silence`.

	.. versionadded:: /errata-CR-r4
		Уточнение про то, что "может видеть" касается любой возможности точно ощущать субъект.
		Это касается например :ref:`skill--Intimidation--Coerce`, :ref:`action--Investigator--Devise-a-Stratagem` или :ref:`action--Thaumaturge--Exploit-Vulnerability`.
...
* Восстановите свои 3 действия и 1 реакцию. Если вы не потратили свою реакцию с прошлого хода - вы теряете ее, так как не можете сохранить действия или реакции с одного хода, чтобы использовать их в другом. Некоторые умения или состояния (такие как :c_quickened:`ускорен`, :c_slowed:`замедлен` или :c_stunned:`ошеломлен`) могут менять количество получаемых действий и то, восстановите ли вы свою реакцию. Если вы теряете действия и получаете дополнительные действия (как если бы вы были одновременно :c_quickened:`ускорены` и :c_slowed:`замедлены`), то решаете какие действия терять. Более подробно это описано в сноске :ref:`Получение и потеря действий (Gaining and Losing Actions) <sidebar--Gaining-and-Losing-Actions>`.

.. versionchanged:: /errata-CR-r4
	Уточнение, что вы восстанавливаете действия (если только этому не мешает состояние), просто не можете ими пользоваться.
...
| **Успех**: Вы наносите урон, в соответствии оружию или безоружной атаке, включая любые имеющиеся у вас модификаторы, бонусы и штрафы на урон.
...
.. sidebar:: Разговор (Speaking)
	...
	Вся речь имеет :t_auditory:`слуховой` признак.
...
| **Критический провал**: Вы не освобождаетесь и не можете попытаться Вырваться снова до своего следующего хода.
...
Мастер может решить, что вы должны делать Поиск в виде активности, тратя больше действий или даже минуты или часы, если ищете в особенно захламленной области.
...
Если у вас есть штраф множественной атаки и подготовленное вами действие является :t_attack:`атакующим`, то эта подготовленная атака получает штраф множественной атаки, соответствующий времени когда вы использовали Приготовиться.
...
Передвижение в столкновениях (`Movement in Encounters <https://2e.aonprd.com/Rules.aspx?ID=443>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
Принудительное передвижение (`Forced Movement <https://2e.aonprd.com/Rules.aspx?ID=451>`_)
""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""
...
Если принудительное передвижение сдвинет вас в пространство, которое вы не можете занять, так как например, на пути объект или у вас нет нужного вида перемещения, то вы останавливаетесь в последнем месте которое можете занять.
Обычно существо или эффект, принуждающие к движению, выбирают путь, по которому пойдет жертва.
...
.. admonition:: Официальное пояснение (4 издание)
	...
	**Вопрос**: Как мне рассудить принудительное передвижение, если существо заставляет другое существо двигаться вместе с ним?

	**Ответ**: Существа, такие как :doc:`/creatures/bestiary/Elephant/Elephant` и :doc:`/creatures/bestiary/Roc` могут двигаться беря с собой :c_grabbed:`схваченное` существо.
	Теперь берем во внимание 2-й абзац.
	Можете ли вы призвать слона и заставить его бежать вдоль :ref:`spell--w--Wall-of-Fire` и тащить существо на бивнях по каждому квадрату этой стены?
	Определенно нет.
	Суть в том, что эта жертва переносится существом.
	Однако, Мастер может рассудить разумные заявки.
	Например, если слон двигается **через** эту *стену огня*, то и он и то существо оба получают урон.
...
Существо :c_flat_footed:`застигнуто врасплох` (получая штраф обстоятельства -2 к КБ) для атак ближнего боя от тех существ, которые берут его в тиски.
...
.. versionchanged:: /errata-CR-r3
	Теперь явно указано, что взятие в тиски работает только для атак ближнего боя.
...
Если вы были :s_medium:`среднего` размера на ездовом животном :s_large:`большого` размера, то можете атаковать существо с одной стороны вашего скакуна, а следующим действием атаковать с другой стороны.
Если у вас досягаемость больше обычной, дистанция частично зависит от размера ездового животного.
На скакуне :s_medium:`среднего` размера и менее, вы используете свою нормальную досягаемость.
На :s_large:`большом` или :s_huge:`огромном` скакуне, вы можете атаковать любой квадрат смежный с ездовым животным, если ваша досягаемость 5 или 10 футов, или любой квадрат в пределах 10 футов от скакуна (включая по диагонали), если у вас досягаемость 15 футов.
...
.. sidebar:: Активности исследования у навыков (Skill Exploration Activities)
	...
	:ref:`activity--gen--Decipher-Writing`: Вы используете подходящий навык, чтобы понять архаические, эзотерические или запутанные тексты.
...
Вы ищете информацию о окружении пока путешествуете с половиной скорости.
...
Большинство эффектов таких заклинаний или предметов могут поддерживаться в течение 10 минут, однако некоторые уточняют, что могут поддерживаться в течение другого времени.
...
.. sidebar:: Активности отдыха у навыков (Skill Downtime Activities)
	...
	:ref:`downtime--Subsist`: Вы ищете еду и кров в дикой природе или в поселении.
