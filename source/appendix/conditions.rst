...
Если зрение ваше единственное точное чувство, все существа и объекты :c_concealed:`скрыты` для вас.
...
Вы получаете штраф состояния -2 к проверкам Восприятия на инициативу и проверкам связанным со звуком, но также полагающимся на другие сенсорные чувства.
...
.. sidebar:: Группы состояний (Groups of Conditions)
	...
	| **Стадии обнаружения**: :c_observed:`замечен`, :c_hidden:`спрятан`, :c_undetected:`необнаружен`, :c_unnoticed:`незамечен`.
	| **Чувства**: :c_blinded:`слепота`, :c_concealed:`скрыт`, :c_dazzled:`ослеплен`, :c_deafened:`глухота`, :c_invisible:`невидимый`.
	| **Смерть и умирание**: :c_doomed:`обречен`, :c_dying:`при смерти`, :c_unconscious:`без сознания`, :c_wounded:`ранен`.
	| **Отношения**: :c_hostile:`враждебный`, :c_unfriendly:`недружественный`, :c_indifferent:`безразличный`, :c_friendly:`дружественный`, :c_helpful:`любезный`.
	| **Понижающие характеристики**: :c_enfeebled:`ослаблен`, :c_clumsy:`неуклюжесть`, :c_drained:`истощен`, :c_stupefied:`одурманен`.


	.. rubric:: ПРАВИЛА О СМЕРТИ И УМИРАНИЕ (DEATH AND DYING RULES)

	Состояния :c_doomed:`обречен`, :c_dying:`при смерти`, :c_unconscious:`без сознания` и :c_wounded:`ранен` относятся к процессу смерти.
	...
	* Вы получаете состояние :c_dying:`при смерти 1`. Если результат эффекта, от которого вы потеряли сознание, был критическим успехом со стороны атаковавшего или результатом вашего критического провала, то вместо этого вы получаете :c_dying:`при смерти 2`. Если у вас есть состояние :c_wounded:`ранен`, увеличьте значение :c_dying:`при смерти` на величину состояния :c_wounded:`ранен`. Если урон был от :w_nonlethal:`несмертельное` атаки или эффекта, то вы не получаете состояние :c_dying:`при смерти`, а находитесь :c_unconscious:`без сознания` с 0 ОЗ.
...
Если вы получите это состояние, когда :ref:`Карабкаетесь (Climbing) <skill--Athletics--Climb>` или :ref:`Летите (Flying) <action--Fly>`, то вы падаете (см. раздел :ref:`ch9--Falling`).
Вы не можете получить состояние ничком когда :ref:`Плаваете (Swimming) <skill--Athletics--Swim>`.
...
Состояние испуга снижается на 1 в конце каждого вашего хода, если не сказано иначе.
...
Вместо получения продолжительного урона мгновенного, вы получаете его в конце каждого своего хода, бросая кости урона заново, пока у вас не пропадет состояние.
После того как вы получаете продолжительный урон, совершите чистую проверку КС 15, чтобы узнать оправились ли вы от продолжительного урона.
При успехе, состояние заканчивается.
...
.. sidebar:: Правила продолжительного урона (Persistent Damage Rules)

	Дополнительные правила представленные далее применяются в определенных случаях.

	Продолжительный урон проходит и автоматически заканчивается через определенное время, так как огонь тухнет, кровь сворачивается и т.п.
	То, когда это происходит решается Мастером, но обычно на это требуется 1 минута.

	.. _Persistent-Damage--Assisted-Recovery:
	...
	* Действие для помощи может потребовать проверку навыка или другого броска, чтобы определить его эффективность.
	...
	Если вы получите более одного состояния продолжительного урона с уроном одного типа, то большее значение заменит меньшее.
	Весь получаемый вами продолжительный урон наносится одновременно, поэтому, если что-то реагирует на получение вами урона, этот триггер происходит только раз.
	Например, если вы :c_dying:`при смерти` с несколькими типами продолжительного урона, то весь этот урон увеличивает значение состояния :c_dying:`при смерти` только один раз.

	.. rst-class:: clarification
	.. admonition:: Официальное пояснение (4 издание)

		**Вопрос**: Как понять, величина какого продолжительного урона больше если это не очевидно?

		**Ответ**: Вам нужно оставить только наибольшую величину продолжительного урона.
		Как выбрать между 3 продолжительного урона огнем и 1d6 продолжительного урона огнем (который обычно больше, но не всегда)?
		В таких случаях, Мастер должен быстро решить согласно своим догадкам.
		Не переживайте о вычислениях среднего урона, просто следуйте чутью и выберите то что кажется наибольшим.
...
.. _sidebar--Gaining-and-Losing-Actions:
.. sidebar:: Получение и потеря действий (Gaining and Losing Actions)

	:c_quickened:`Ускорен`, :c_slowed:`замедлен` и :c_stunned:`ошеломлен` - основные способы чтобы получить или потерять действия для хода.
	Правила об этом описаны в разделе :ref:`ch9--Actions--Gaining-and-Losing`.
	Вкратце, эти состояния меняют количество действий получаемых в начале вашего хода, таким образом получение состояния в середине вашего хода не изменяет количество действий на этом ходу.
	Если у вас есть конфликтующее состояние, которое воздействует на ваше количество действий, вы выбираете какие из них потерять.
	Например, действие полученное от заклинания :ref:`spell--h--Haste` позволяет вам использовать его только для :ref:`Перемещения (Stride) <action--Stride>` и :ref:`Удара (Strike) <action--Strike>`, так что, если вам требуется лишиться 1 действия из-за состояния :c_slowed:`замедлен`, вы можете решить потерять действие от *ускорения*, оставив другие действия которые можно использовать более гибко.

	Некоторые состояния не позволяют вам совершать определенный набор действий, как правило реакции.
	Другие состояния просто говорят что вы не можете действовать.
	Когда вы не можете действовать, то вы не способны совершать никаких действий.
	В отличие от :c_slowed:`замедлен` или :c_stunned:`ошеломлен`, они не изменяют количество получаемых действий; они просто препятствуют их использованию.
	Это значит что, если вы как-либо были избавлены от :c_paralyzed:`паралича` во время своего хода, то можете сразу действовать.
...
Например, если вы были :c_stunned:`ошеломлен 4`, вы потеряете все 3 действия в ваш ход, уменьшив значение состояния до :c_stunned:`ошеломлен 1`; на следующем ходу вы потеряете еще 1 действие и сможете использовать оставшиеся 2 как обычно.
Ошеломление также может иметь продолжительность вместо значения, как например ":c_stunned:`ошеломлен` на 1 минуту".
В этом случае вы теряете все ваши действия в течение указанного времени.

Ошеломление замещает состояние :c_slowed:`замедлен`.
Если продолжительность ошеломления заканчивается когда вы :c_slowed:`замедлены`, вы считаете потерю действий от :c_stunned:`ошеломления` в пользу :c_slowed:`замедления`.
Например, если вы были :c_stunned:`ошеломлены 1` и :c_slowed:`замедлены 2` в начале хода, вы потеряете 1 действие от ошеломления и 1 от замедления, так что у вас останется 1 действие на этом ходу.
...
.. sidebar:: Избыточные состояния (Redundant Conditions)
	...
	Перед окончанием следующего хода этого монстра, ловушка отравляет вас, делая вас :c_enfeebled:`ослабленным 2` на 1 минуту.
...
Когда вы ослаблены, вы получаете штраф состояния равный значению состояния к броскам основанным на Силе, и КС, включая атаки ближнего боя основанные на Силе, броски урона основанные на Силе, и проверки Атлетики.
...
Существо, от которого вы спрятались, является для вас застигнутым врасплох, а также должно пройти чистую проверку с КС 11 когда делает вас целью для атаки, заклинания или другого эффекта, иначе этот эффект не действует на вас.
Эффекты по области не подвержены этой чистой проверке.

Существо может попытаться использовать действие :ref:`action--Seek` чтобы :c_observed:`заметить` вас, как описано.
